...
describe 'opensearch::license', type: 'class' do
  ...
  on_supported_os.to_a.sample(1).to_h.each do |os, facts|
    context "on #{os}" do
      ...
      context 'when managing x-pack license' do
        ...
        it do
          expect(subject).to contain_class('opensearch::license')
        end

        it do
          expect(subject).to contain_es_instance_conn_validator(
            'license-conn-validator'
          ).that_comes_before('opensearch_license[xpack]')
        end

        it do
          expect(subject).to contain_opensearch_license('xpack').with(
            ensure: 'present',
            content: {
              'license' => {
                'uid' => 'cbff45e7-c553-41f7-ae4f-9205eabd80xx',
