...
shared_examples 'plugin provider' do |version|
  describe "opensearch #{version}" do
    before do
    ...
    describe 'setup' do
      ...
      describe 'configdir' do
        it 'sets the ES_PATH_CONF env var' do
          resource[:configdir] = '/etc/opensearch'
          expect(provider.with_environment do
            ENV['ES_PATH_CONF']
          end).to eq('/etc/opensearch')
        end
      end
    end
    ...
    describe 'java_home unset' do
      ...
      it 'defaults to the opensearch bundled JDK' do
        resource[:java_home] = ''
        expect(provider.with_environment do
          ENV['JAVA_HOME']
        end).to eq(opensearch_java_home)
      end
    end
