...
describe 'opensearch::template', type: 'define' do
  on_supported_os(
    ...
    context "on #{os}" do
      ...
      describe 'template from source' do
        ...
        it { is_expected.to contain_opensearch__template('foo') }

        it do
          expect(subject).to contain_es_instance_conn_validator('foo-template').
            that_comes_before('Opensearch_template[foo]')
        end

        it 'passes through parameters' do
          expect(subject).to contain_opensearch_template('foo').with(
            ensure: 'present',
            source: 'puppet:///path/to/foo.json',
            protocol: 'https',
            host: '127.0.0.1',
      ...
      describe 'template deletion' do
        ...
        it 'removes templates' do
          expect(subject).to contain_opensearch_template('foo').with(ensure: 'absent')
        end
      end
    end
  end
