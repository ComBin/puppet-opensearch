...
describe 'opensearch::pipeline', type: 'define' do
  ...
  on_supported_os(
    ...
    context "on #{os}" do
      ...
      describe 'class parameter inheritance' do
        ...
        it do
          expect(subject).to contain_opensearch__pipeline(title)
          expect(subject).to contain_es_instance_conn_validator("#{title}-ingest-pipeline").
            that_comes_before("opensearch_pipeline[#{title}]")
          expect(subject).to contain_opensearch_pipeline(title).with(
            ensure: 'present',
            content: {},
            protocol: 'https',
            host: '127.0.0.1',
      ...
      describe 'pipeline deletion' do
        ...
        it 'removes pipelines' do
          expect(subject).to contain_opensearch_pipeline(title).with(ensure: 'absent')
        end
      end
    end
  end
